...
namespace KPCodeGen.Core.Reader
{
    ...
    /// </summary>
    public class SybaseMetadataReader : AbstractMetadataReader
    {
                ...
                try
                {
                    using (var tableDetailsCommand = sqlCon.CreateCommand())
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            var mappedType = m.MapFromDBType(DatabaseServerType.Sybase, dataType, dataLength, dataPrecision,
                                                             dataScale, isPrimaryKey.Value);
                ...
                try
                {
                    using (var tableDetailsCommand = sqlCon.CreateCommand())
                    {
                        ...
                        using (var sqlDataReader = tableDetailsCommand.ExecuteReader(CommandBehavior.Default))
                        {
                            ...
                            {
                                string tableName = sqlDataReader.GetString(0).Replace("'", string.Empty);
