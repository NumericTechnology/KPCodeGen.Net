...
namespace KPCodeGen.Core.Reader
{
    ...
    public class OracleMetadataReader : AbstractMetadataReader
    {
...
order by column_id";
                    ...
                    using (OracleDataReader oracleDataReader = tableCommand.ExecuteReader(CommandBehavior.Default))
                    {
                        ...
                        while (oracleDataReader.Read())
                        {
                            ...
                            bool isPrimaryKey = ConstraintTypeResolver.IsPrimaryKey(constraintType);
                            ...
                            columns.Add(new Column(table)
                            {
                                ...
                                MappedDataType = m.MapFromDBType(DatabaseServerType.Oracle, oracleDataReader.GetOracleString(1).Value, dataLength, dataPrecision, dataScale, isPrimaryKey),
        ...
        public override List<Table> GetTables(string owner)
        {
            ...
            using (conn)
            {
                using (OracleCommand tableCommand = conn.CreateCommand())
                {
                    tableCommand.CommandText = @"select table_name from all_tables where all_tables.owner = :table_name
                                                union all
                                                select view_name as table_name from all_views where all_views.owner = :table_name
                                                order by table_name
                                                ";
